...
SNPs::SNPs(Fgwas_params *p){
	...
	//read input file
	if (!params->pairwise) load_snps_z(params->infile, params->V, params->wannot, params->dannot, params->segannot);
	else if (params->pairwise) load_snps_pw(params->infile, params->wannot, params->dannot, params->segannot);
...
void SNPs::load_snps_pw(string infile, vector<string> annot, vector<string> dannot, vector<string> segannot){
	igzstream in(infile.c_str()); //only gzipped files
	vector<string> line;
	struct stat stFileInfo;
	int intStat;
    ...
    while(getline(in, st)){
    	...
    	SNP s(rs, chr , pos, bf1, bf2, an, dists, dmodels);
    	if (params->finemap){
    		int snumber = atoi(line[segnumberindex].c_str());
    		s.chunknumber = snumber;
    	}

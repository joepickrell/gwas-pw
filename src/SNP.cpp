...
SNP::SNP(string rs, string c, int p, double b1, double b2, vector<bool> an, vector<int> ds, vector<vector<pair<int, int> > > dmodels){
	//for pairwise
	id = rs;
	chr = c;
	pos = p;
	f = 0; //set to zero, should not be used
	N = 0; //set to zero, should not be used
	Z = 0; // ditto
	V = 0; // ditto
	W = 0; // ditto
	BF = b1;
